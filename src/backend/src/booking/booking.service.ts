...
listAvailableRooms(timeslotId: number, candidateRoomIds: number[]): number[] | Promise<number[]> {
 
  if (Array.isArray((this as any).bookings)) {
    const arr = (this as any).bookings as Array<{ roomId: number; timeslotId: number; status?: string }>;
    return candidateRoomIds.filter(roomId =>
      !arr.some(b => b.roomId === roomId && b.timeslotId === timeslotId && b.status !== 'cancelled'),
    );
  }
